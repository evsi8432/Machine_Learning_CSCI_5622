...
class LogReg:
    ...
    def sgd_update(self, x_i, y):
        ...
        pi_i = sigmoid(np.dot(x_i,self.w))
        
        for j in range(len(self.w)):
            self.w[j] = self.w[j] + self.eta*(y - pi_i)*x_i[j]
...
def sigmoid(score, threshold=20.0):
    ...
    return np.exp(score)/(1.0+np.exp(score))
...
if __name__ == "__main__":
    ...
    for epoch in range(args.passes):
        ...
        moder = 0
        for x_i,y in zip(data.train_x,data.train_y):
            lr.w = lr.sgd_update(x_i,y)
            if moder % 50 == 0:
