...
def split_cv(length, num_folds):
    ...
    index_starts = [int(round(x)) for x in np.linspace(0,length,num_folds+1)]
    folds = [indices[index_starts[i]:index_starts[i+1]] for i in range(len(index_starts)-1)]
    
    tests = folds
    trains = []
    for fold_excluded in folds:
        train = []
        for fold_included in folds:
            if fold_included is not fold_excluded:
                train += fold_included
        trains.append(train)
    
    splits = [SplitIndices(trains[i], tests[i]) for i in range(num_folds)]
