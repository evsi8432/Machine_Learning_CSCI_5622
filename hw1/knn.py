...
class Knearest:
    ...
    def __init__(self, x, y, k = 5):
        ...
        ### start Evan's code (no need to do anything) ###
    ...
    def majority(self, item_indices):
        ...
        labels = list([self._y[ind] for ind in list(item_indices)])
		
        counts = [labels.count(x) for x in labels]
        maxcount = max(counts)
		
        modes = []
        for label in labels:
            if labels.count(label) == maxcount and label not in modes:
                modes.append(label)
    ...
    def confusion_matrix(self, test_x, test_y, debug=False):
        ...
        for xx, yy in zip(test_x, test_y):
            ...
            real_label = yy
            if real_label not in conf_mat:
                conf_mat[real_label] = {}
				
            guessed_label = self.classify(xx)
            if guessed_label in conf_mat[real_label]:
                conf_mat[real_label][guessed_label] += 1
            else:
                conf_mat[real_label][guessed_label] = 1
